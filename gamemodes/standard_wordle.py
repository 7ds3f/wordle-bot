...
async def run(ctx, interaction, users):
    ...
    if interaction != None:
        print(f"[INFO] {interaction.user} started a new Wordle game (mode=Standard Wordle, hidden_word={hidden_word})")
    ...
    while guesses_rem > 0:
        ...
        while guess.content == "" or guess.content[0] == "!":
            if guess.content == "!q":
                print(f"[INFO] {ctx.author} quit their Wordle game (mode=Standard Wordle, hidden_word={hidden_word})")
